...
def test_advisor_interface_summary(mocker, mock_df):
    mocker.patch('scripts.financial_summary.generate_financial_summary', return_value="Mocked summary")
    result = advisor_interface(mock_df, "Summary", 26, "", "")
    assert result == "Mocked summary"

def test_advisor_interface_advice(mocker, mock_df):
    mocker.patch('scripts.financial_advice.generate_personalized_advice', return_value="Mocked advice")
